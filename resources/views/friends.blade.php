        ...
        <table class="mt-4 w-full min-w-max table-auto text-left">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="p-4 border-b border-blue-gray-50">
                        <form action="{{ route('acceptFriendRequest', ['friendId' => $pendings->user->id]) }}" method="POST">
                            @csrf
                            <input type="hidden" name="userId" value="{{ $pendings->user->id }}">
                            <button type="submit">Accepter</button>
                        </form>
                        <form action="{{ route('rejectFriendRequest', ['friendId' => $pendings->user->id]) }}" method="POST">
                            @csrf
                            <input type="hidden" name="userId" value="{{ $pendings->user->id }}">
